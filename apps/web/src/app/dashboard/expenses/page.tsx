...
export default function ExpensesPage() {
  ...
  const allItems = expenses.flatMap((exp) =>
    exp.items
      ...
      .filter((item) => !itemSearch || item.displayName.toLowerCase().includes(searchLower) || item.rawName.toLowerCase().includes(searchLower))
      .map((item) => ({
        ...item,
        expenseId: exp.id,
  ...
  const handleExport = async (format: 'csv' | 'xlsx' | 'pdf') => {
    setExporting(true);
    try {
      // If all data fits on one page, use what we already have
      ...
      if (format === 'csv') exportCSV(exportExpenses, opts);
      else if (format === 'xlsx') await exportXLSX(exportExpenses, opts);
      else if (format === 'pdf') await exportPDF(exportExpenses, opts);
    } catch (err: any) {
    ...
    <div className="page-container space-y-4">
      ...
      <div className="card-elevated !p-3">
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input type="date" className="input-clean w-auto text-xs" value={dateFrom}
              onChange={(e) => { setDateFrom(e.target.value); setPage(1); }} />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">To</label>
            <input type="date" className="input-clean w-auto text-xs" value={dateTo}
              onChange={(e) => { setDateTo(e.target.value); setPage(1); }} />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Category</label>
            <select className="select-clean w-auto text-xs" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
          ...
          <div>
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Item Search</label>
            <input type="text" className="input-clean w-32 text-xs" placeholder="e.g. potato" value={itemSearch}
              onChange={(e) => setItemSearch(e.target.value)} />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Min Amount</label>
            <input type="number" className="input-clean w-24 text-xs" placeholder="0" value={minAmount}
              onChange={(e) => { setMinAmount(e.target.value); setPage(1); }} />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Max Amount</label>
            <input type="number" className="input-clean w-24 text-xs" placeholder="Any" value={maxAmount}
              onChange={(e) => { setMaxAmount(e.target.value); setPage(1); }} />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Sort</label>
            <select className="select-clean w-auto text-xs" value={sort} onChange={(e) => setSort(e.target.value)}>
          ...
          <div className="ml-auto">
            <div className="dropdown dropdown-end">
              ...
              <ul tabIndex={0} className="dropdown-content menu rounded-lg border border-gray-200 bg-white p-1 shadow-lg z-50 w-36">
                <li><button onClick={() => handleExport('csv')} className="text-xs">CSV</button></li>
                <li><button onClick={() => handleExport('xlsx')} className="text-xs">Excel</button></li>
                <li><button onClick={() => handleExport('pdf')} className="text-xs">PDF</button></li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="card-elevated animate-pulse space-y-3">
          {[...Array(8)].map((_, i) => <div key={i} className="h-8 rounded bg-gray-50" />)}
        </div>
      ) : allItems.length === 0 ? (
        ...
        <div className="card-elevated overflow-x-auto !p-0">
          <table className="w-full text-left">
            ...
            <tbody>
              {allItems.map((item) => (
                <tr key={`${item.expenseId}-${item.id}`} className="border-b border-gray-50 text-sm hover:bg-gray-50/50">
                  <td className="px-3 py-2 text-xs text-gray-500 whitespace-nowrap">{formatDate(item.expenseDate)}</td>
                  ...
                  <td className="px-3 py-2 text-right text-xs tabular-nums text-gray-600">{item.quantity}</td>
                  ...
                  <td className="px-3 py-2 text-right text-xs tabular-nums text-gray-500">
                    {item.unitPrice ? formatCurrency(item.unitPrice, item.currency) : item.quantity > 0 ? formatCurrency(item.amount / item.quantity, item.currency) : '-'}
                  </td>
                  <td className="px-3 py-2 text-right font-medium tabular-nums text-gray-900">{formatCurrency(item.amount, item.currency)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
