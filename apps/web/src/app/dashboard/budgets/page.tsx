...
import { useBudgets, useBudgetProgress, useCreateBudget, useDeleteBudget, useCategories, useInsightSummary } from '@/hooks/use-expenses';
...
export default function BudgetsPage() {
  ...
  const totalSpent = progress.length > 0
    ...
    <div className="page-container max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="stat-card">
          ...
          <p className={`mt-1 text-xl font-bold ${totalBudget - totalSpent >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
      ...
      <form onSubmit={handleCreate} className="card-elevated space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
          <div className="sm:col-span-4">
            ...
            <select className="select-clean" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>
          ...
          <div className="sm:col-span-3">
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Monthly Limit (INR)</label>
            <input type="number" step="100" className="input-clean" placeholder="5000" value={limit} onChange={(e) => setLimit(e.target.value)} required />
          </div>
          <div className="sm:col-span-3">
            <label className="mb-1 block text-[10px] font-medium uppercase text-gray-400">Period</label>
            <div className="flex items-center gap-3 pt-1">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input type="radio" name="period" className="radio radio-primary radio-xs" checked={isRecurring} onChange={() => setIsRecurring(true)} />
                <span className="text-xs text-gray-600">Every month</span>
              </label>
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input type="radio" name="period" className="radio radio-primary radio-xs" checked={!isRecurring} onChange={() => setIsRecurring(false)} />
                <span className="text-xs text-gray-600">One-time</span>
              </label>
            </div>
            {!isRecurring && (
              <input type="month" className="input-clean mt-1 text-xs" value={month} onChange={(e) => setMonth(e.target.value)} />
            )}
          </div>
          <div className="flex items-end sm:col-span-2">
            <button type="submit" className="btn btn-primary btn-sm w-full" disabled={!limit || createBudget.isPending}>
              {createBudget.isPending ? <span className="loading loading-spinner loading-xs" /> : 'Set'}
            </button>
          </div>
        </div>
      </form>
      ...
      {progress.length > 0 && (
        <div className="card-elevated space-y-4">
          ...
          {progress.map((b) => {
            ...
            const status = b.percentage >= 100 ? 'Over budget!' : b.percentage >= 90 ? 'Almost there' : b.percentage >= 70 ? 'Watch out' : 'On track';
            return (
      ...
      {budgets.length > 0 && (
        <div className="card-elevated">
          ...
          <div className="space-y-2">
            {budgets.map((b: any) => (
              <div key={b.id} className="flex items-center gap-3 rounded-lg border border-gray-50 bg-gray-50 px-3 py-2">
                <span>{b.categoryIcon ?? '📊'}</span>
      ...
      <ConfirmDialog
        ...
        confirmLabel="Remove"
        onConfirm={() => { if (deleteTarget) deleteBudget.mutate(deleteTarget, { onSuccess: () => setDeleteTarget(null) }); }}
